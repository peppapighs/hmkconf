...
export function CompatibilityDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      if (configFile.profiles) {
        ...
        const availableSettings = configFile.settings
          ? [
              "keymap",
              "actuationMap",
              "advancedKeys",
        ...
        setSelectedSettings(availableSettings)
        setSelectAllChecked(availableSettings.length > 0)
      }
    }
  }, [open, configFile])
  ...
  const toggleSetting = (setting: string) => {
    setSelectedSettings((prev) => {
      ...
      const availableSettings = configFile.settings
        ? Object.keys(configFile.settings).filter(
            (key) =>
              configFile.settings &&
  ...
  const getSettingDisplayName = (key: string): string => {
    switch (key) {
      case "keymap":
        return "Keymap"
      case "actuationMap":
        return "Actuation Settings"
      case "advancedKeys":
        return "Advanced Key Settings"
      case "tickRate":
        return "Tick Rate"
      case "calibration":
        return "Calibration"
      case "profiles":
        return "All Profiles (0-4)"
      default:
        return key
    }
  }
        ...
        <div className="py-2">
          <div className="mb-4 rounded-md border bg-muted/30 p-3">
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
              <div className="col-span-2 mb-1 font-medium">
              ...
              <div className="font-medium">
                v{configFile.deviceInfo.firmwareVersion}
              </div>
                    ...
                    >
                      ...
                      >
                        {selectedSettings.includes("profiles") ? (
                          ...
                          <div className="mr-2 size-4 rounded-sm border" />
                        )}
                        All Profiles (0-4)
                      </Button>
                    </div>
